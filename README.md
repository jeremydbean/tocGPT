...
### Codex prompt to generate a repair script
Paste the prompt below into Codex to generate a PowerShell script (`docker_wsl_repair.ps1`) that diagnoses and fixes the issue automatically.

```
You are to generate a PowerShell fix script that resolves the following Windows Docker Desktop problem:
...
2. Ensure WSL is enabled:
   - Enable-WindowsOptionalFeature -Online -FeatureName Microsoft-Windows-Subsystem-Linux
   - Enable-WindowsOptionalFeature -Online -FeatureName VirtualMachinePlatform
3. Ensure the wsl kernel is installed and updated:
   - wsl --update
4. Ensure that a WSL2 Linux distribution exists and is set to version 2:
   - wsl -l -v
   - wsl --set-default-version 2
5. Restart the LxssManager service.
6. Restart Docker Desktop automatically.
7. Wait until com.docker.service is reporting a running state.
8. Confirm Docker is responding to:
   - docker version
   - docker info
   - docker run hello-world

The output script must:
• Be a single PowerShell file
• Contain no placeholders
• Contain no interactive prompts
• Echo status messages as it runs
• Exit with non-zero code if Docker still fails after the fix.

After generating the script, Codex must also output a README.md update describing:
• Why the error happens
• How to run the fix script
• How to validate Docker is running
```

### Quick fix (new install or broken environment)
Run the repair script generated by Codex:

```powershell
./docker_wsl_repair.ps1
```

The script enables required Windows features, updates WSL, restarts `LxssManager`, restarts Docker Desktop, and verifies engine health.
